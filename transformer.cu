...
struct MLP_CUDA {
    ...
    void save_weights(const std::string& filename) {
        ...
        if (!out.is_open()) {
            std::cerr << "Error guardando modelo: " << filename << "\n";
            return;
        }
        ...
        cudaMemcpy(hW1.data(), L1.W, sizeof(double)*W1_size, cudaMemcpyDeviceToHost);
        cudaMemcpy(hb1.data(), L1.b, sizeof(double)*b1_size, cudaMemcpyDeviceToHost);

        out.write((char*)hW1.data(), sizeof(double)*W1_size);
        out.write((char*)hb1.data(), sizeof(double)*b1_size);
        ...
        cudaMemcpy(hW2.data(), L2.W, sizeof(double)*W2_size, cudaMemcpyDeviceToHost);
        cudaMemcpy(hb2.data(), L2.b, sizeof(double)*b2_size, cudaMemcpyDeviceToHost);

        out.write((char*)hW2.data(), sizeof(double)*W2_size);
        out.write((char*)hb2.data(), sizeof(double)*b2_size);

        out.close();
        std::cout << "Modelo guardado en " << filename << "\n";
    }
...
int main() {
    NSLKDD train, test;
    train.load_file("NSL_KDD-master/KDDTrain+.txt");
    test.load_file("NSL_KDD-master/KDDTest+.txt");

    train.process();
    test.process();

    cout << "Train size: " << train.X_tokens.size() << "\n";
    cout << "Test size : " << test.X_tokens.size() << "\n";
    cout << "Classes   : " << train.num_classes() << "\n";

    int P = train.proto_vocab.size();
    int S = train.service_vocab.size();
    int F = train.flag_vocab.size();
    ...
    TokenEmbedding emb(T0_dim, T1_dim, T2_dim, T3_dim, d_model);
    Transformer transformer(seq_len, d_model, heads, layers);
    ...
    MLP_CUDA mlp(
        64,                 // input_dim = CLS
        128,                // hidden
        train.num_classes(),// output_dim
        batch_size
    );

    cout << "\n==== Entrenando Transformer + MLP CUDA ====\n";

    int N = train.X_tokens.size();
    vector<int> idx(N);
    iota(idx.begin(), idx.end(), 0);

    vector<vector<double>> Xb(batch_size, vector<double>(64));
    vector<int> Yb(batch_size);
    ...
    for (int ep = 1; ep <= epochs; ep++) {
        ...
        for (int b = 0; b < num_batches; b++) {
            ...
            for (int i = 0; i < batch_size; i++) {
                ...
                int k = idx[b * batch_size + i];

                auto tok_emb = emb.forward(train.X_tokens[k]);

                auto H = transformer.forward(tok_emb);
            ...
            mlp.forward_batch(Xb, Yb);

            double L = mlp.get_loss();
            epoch_loss += L;
    ...
    for (int i = 0; i < test.X_tokens.size(); i++) {

        auto tok_emb = emb.forward(test.X_tokens[i]);
        auto H = transformer.forward(tok_emb);
        vector<double> cls = H[0];

        int pred = mlp.predict(cls);
        if (pred == test.y[i]) correct++;
    }

    double acc = 100.0 * correct / test.X_tokens.size();
    cout << "Test Accuracy = " << acc << "%\n";

    mlp.save_weights("mlp_model.bin");
